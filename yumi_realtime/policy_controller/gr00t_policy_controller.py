...
from gr00t.utils.eval_wrapper import GROOTWrapper
...
class YuMiGROOTPolicyController(YuMiJointAngleROSInterface):
    """YuMi controller for GROOT policy control."""
    ...
    def __init__(self, ckpt_path: str = None, ckpt_id: int = 0, text_prompt: str = None, collect_data: bool = False, debug_mode = False, *args, **kwargs):
        ...
        assert ckpt_path is not None, "GROOT Policy checkpoint path must be provided."
        
        # Setup Diffusion Policy module and weights
        self.model = GROOTWrapper(model_ckpt_folder=ckpt_path, ckpt_id=ckpt_id, text_prompt=text_prompt, device="cuda")
        ...
        logger.info("GROOT Policy controller initialized")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            action_prediction = self.model(input) # GROOT inference step
    ...
    def _yumi_control(self, action, rate = None):
        ...
        l_gripper_cmd = mapped_action[14]  # Left gripper is at index 14
        r_gripper_cmd = mapped_action[15]  # Right gripper is at index 15
...
def main(
    ckpt_path: str = "/home/xi/checkpoints/yumi_coffee_maker/groot/", 
    ckpt_id: int = 10000,
    ...
    yumi_interface = YuMiGROOTPolicyController(ckpt_path, ckpt_id, task_name, collect_data, debug_mode)
